...
add_filter( 'wpcf8_spam', 'wpcf8_akismet' );

function wpcf8_akismet( $spam ) {
	if ( $spam ) {
		return $spam;
	}

	if ( ! wpcf8_akismet_is_available() ) {
		return false;
	}

	if ( ! $params = wpcf8_akismet_submitted_params() ) {
		return false;
	}
	...
	return wpcf8_akismet_comment_check( $c );
}

function wpcf8_akismet_is_available() {
...
function wpcf8_akismet_submitted_params() {
	$params = array(
		'author' => '',
		'author_email' => '',
		'author_url' => '',
	...
	foreach ( (array) $_POST as $key => $val ) {
		if ( '_wpcf8' == substr( $key, 0, 6 ) || '_wpnonce' == $key ) {
			continue;
		}

		if ( is_array( $val ) ) {
			$val = implode( ', ', wpcf8_array_flatten( $val ) );
		}
		...
		if ( $tags = wpcf8_scan_form_tags( array( 'name' => $key ) ) ) {
			$tag = $tags[0];
...
function wpcf8_akismet_comment_check( $comment ) {
	global $akismet_api_host, $akismet_api_port;

	$spam = false;
	$query_string = wpcf8_build_query( $comment );
	...
	return apply_filters( 'wpcf8_akismet_comment_check', $spam, $comment );
}
