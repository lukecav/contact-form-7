...
class WPCF8_ContactForm {
	...
	const post_type = 'wpcf8_contact_form';
	...
	public static function get_template( $args = '' ) {
		...
		foreach ( $properties as $key => $value ) {
			$properties[$key] = wpcf8_ContactFormTemplate::get_default( $key );
		}

		$contact_form->properties = $properties;

		$contact_form = apply_filters( 'wpcf8_contact_form_default_pack',
			$contact_form, $args );
	...
	private static function get_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'wpcf8-f%1$d-p%2$d-o%3$d',
				absint( $id ), get_the_ID(), $global_count );
		} else {
			$unit_tag = sprintf( 'wpcf8-f%1$d-o%2$d',
	...
	private function __construct( $post = null ) {
		...
		do_action( 'wpcf8_contact_form', $this );
	}

	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>wpcf8_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'contact-form-7' );
	...
	public function get_properties() {
		...
		$properties = (array) apply_filters( 'wpcf8_contact_form_properties',
			$properties, $this );
	...
	public function locale() {
		if ( wpcf8_is_valid_locale( $this->locale ) ) {
			return $this->locale;
		} else {
			return '';
		}
	...
	public function set_locale( $locale ) {
		...
		if ( wpcf8_is_valid_locale( $locale ) ) {
			$this->locale = $locale;
		} else {
			$this->locale = 'en_US';
		}
	...
	public function is_posted() {
		if ( ! wpcf8_Submission::get_instance() ) {
			return false;
		}

		if ( empty( $_POST['_wpcf8_unit_tag'] ) ) {
			return false;
		}

		return $this->unit_tag == $_POST['_wpcf8_unit_tag'];
	}
	...
	public function form_html( $args = '' ) {
		...
		if ( 'raw_form' == $args['output'] ) {
			return '<pre class="wpcf8-raw-form"><code>'
		...
		$html = sprintf( '<div %s>',
			wpcf8_format_atts( array(
				'role' => 'form',
				'class' => 'wpcf8',
				'id' => $this->unit_tag,
				( get_option( 'html_type' ) == 'text/html' ) ? 'lang' : 'xml:lang'
					=> $lang_tag,
				'dir' => wpcf8_is_rtl( $this->locale ) ? 'rtl' : 'ltr',
			) )
		);

		$html .= "\n" . $this->screen_reader_response() . "\n";

		$url = wpcf8_get_request_uri();
		...
		$url = apply_filters( 'wpcf8_form_action_url', $url );

		$id_attr = apply_filters( 'wpcf8_form_id_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_id'] ) );

		$name_attr = apply_filters( 'wpcf8_form_name_attr',
		...
		$class = implode( ' ', $class );
		$class = apply_filters( 'wpcf8_form_class_attr', $class );

		$enctype = apply_filters( 'wpcf8_form_enctype', '' );
		$autocomplete = apply_filters( 'wpcf8_form_autocomplete', '' );

		$novalidate = apply_filters( 'wpcf8_form_novalidate',
			wpcf8_support_html5() );
		...
		$atts = array(
			...
			'method' => 'post',
			'class' => $class,
			'enctype' => wpcf8_enctype_value( $enctype ),
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_wpcf8' => $this->id,
			'_wpcf8_version' => wpcf8_VERSION,
			'_wpcf8_locale' => $this->locale,
			'_wpcf8_unit_tag' => $this->unit_tag,
		);

		if ( wpcf8_VERIFY_NONCE ) {
			$hidden_fields['_wpnonce'] = wpcf8_create_nonce( $this->id );
		}

		$hidden_fields += (array) apply_filters(
			'wpcf8_form_hidden_fields', array() );
	...
	public function form_response_output() {
		...
		if ( $this->is_posted() ) { // Post response output for non-AJAX
			...
			if ( $submission->is( 'validation_failed' ) ) {
				$class .= ' wpcf8-validation-errors';
			} elseif ( $submission->is( 'spam' ) ) {
				$class .= ' wpcf8-spam-blocked';
			} elseif ( $submission->is( 'mail_sent' ) ) {
				$class .= ' wpcf8-mail-sent-ok';
			} elseif ( $submission->is( 'mail_failed' ) ) {
				$class .= ' wpcf8-mail-sent-ng';
			}
		} else {
			$class .= ' wpcf8-display-none';
		}
		...
		$output = apply_filters( 'wpcf8_form_response_output',
	...
	public function validation_error( $name ) {
		...
		$error = sprintf(
			'<span role="alert" class="wpcf8-not-valid-tip">%s</span>',
			esc_html( $error ) );

		return apply_filters( 'wpcf8_validation_error', $error, $name, $this );
	}

	/* Form Elements */

	public function replace_all_form_tags() {
		$manager = wpcf8_FormTagsManager::get_instance();
		$form = $this->prop( 'form' );

		if ( wpcf8_AUTOP ) {
			$form = $manager->normalize( $form );
			$form = wpcf8_autop( $form );
		}
	...
	public function form_do_shortcode() {
		wpcf8_deprecated_function( __METHOD__, '4.6',
			'wpcf8_ContactForm::replace_all_form_tags' );
	...
	public function form_scan_shortcode( $cond = null ) {
		wpcf8_deprecated_function( __METHOD__, '4.6',
			'wpcf8_ContactForm::scan_form_tags' );

		return $this->scan_form_tags( $cond );
	}

	public function form_elements() {
		return apply_filters( 'wpcf8_form_elements',
	...
	public function collect_mail_tags( $args = '' ) {
		...
		return apply_filters( 'wpcf8_collect_mail_tags', $mailtags );
	}
	...
	public function submit( $ajax = false ) {
		$submission = wpcf8_Submission::get_instance( $this );
		...
		if ( $ajax ) {
			...
			if ( ! empty( $on_submit ) ) {
				$result['scripts_on_submit'] = array_map(
					'wpcf8_strip_quote', $on_submit );
			}
		}

		do_action( 'wpcf8_submit', $this, $result );
	...
	public function message( $status, $filter = true ) {
		...
		if ( $filter ) {
			...
			$message = wpcf8_mail_replace_tags( $message, array( 'html' => true ) );
			$message = apply_filters( 'wpcf8_display_message', $message, $status );
		}
	...
	private function upgrade() {
		...
		if ( is_array( $messages ) ) {
			foreach ( wpcf8_messages() as $key => $arr ) {
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value ) {
				update_post_meta( $post_id, '_' . $prop,
					wpcf8_normalize_newline_deep( $value ) );
			}
			...
			if ( $this->initial() ) {
				$this->id = $post_id;
				do_action( 'wpcf8_after_create', $this );
			} else {
				do_action( 'wpcf8_after_update', $this );
			}

			do_action( 'wpcf8_after_save', $this );
		}
	...
	public function copy() {
		...
		return apply_filters( 'wpcf8_copy', $new, $this );
	}
	...
	public function shortcode( $args = '' ) {
		...
		return apply_filters( 'wpcf8_contact_form_shortcode', $shortcode, $args, $this );
	}
}
