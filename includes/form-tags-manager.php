...
function wpcf8_add_form_tag( $tag, $func, $features = '' ) {
	$manager = wpcf8_FormTagsManager::get_instance();

	return $manager->add( $tag, $func, $features );
}

function wpcf8_remove_form_tag( $tag ) {
	$manager = wpcf8_FormTagsManager::get_instance();

	return $manager->remove( $tag );
}

function wpcf8_replace_all_form_tags( $content ) {
	$manager = wpcf8_FormTagsManager::get_instance();

	return $manager->replace_all( $content );
}

function wpcf8_scan_form_tags( $cond = null ) {
	$contact_form = wpcf8_ContactForm::get_current();
...
function wpcf8_form_tag_supports( $tag, $feature ) {
	$manager = wpcf8_FormTagsManager::get_instance();

	return $manager->tag_type_supports( $tag, $feature );
}

class wpcf8_FormTagsManager {
	...
	private function scan_callback( $m, $replace = false ) {
		...
		if ( is_array( $attr ) ) {
			if ( is_array( $attr['options'] ) ) {
				...
				&& ! empty( $attr['options'] ) ) {
					...
					if ( ! wpcf8_is_name( $scanned_tag['name'] ) ) {
		...
		$scanned_tag = apply_filters( 'wpcf8_form_tag', $scanned_tag, $replace );
	...
	private function parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = wpcf8_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
