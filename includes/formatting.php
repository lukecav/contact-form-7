...
function wpcf8_autop( $pee, $br = 1 ) {
	...
	/* wpcf8: remove select and input */
	$allblocks = '(?:table|thead|tfoot|caption|col|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|form|map|area|blockquote|address|math|style|p|h[1-6]|hr|fieldset|legend|section|article|aside|hgroup|header|footer|nav|figure|figcaption|details|menu|summary)';
	$pee = preg_replace( '!(<' . $allblocks . '[^>]*>)!', "\n$1", $pee );
	$pee = preg_replace( '!(</' . $allblocks . '>)!', "$1\n\n", $pee );

	/* wpcf8: take care of [response], [recaptcha], and [hidden] tags */
	...
	/* wpcf8: take care of [response], [recaptcha], and [hidden] tag */
	...
	if ( $br ) {
		...
		/* wpcf8: remove extra <br /> just added before [response], [recaptcha], and [hidden] tags */
...
function wpcf8_sanitize_query_var( $text ) {
...
function wpcf8_strip_quote( $text ) {
	$text = trim( $text );
...
function wpcf8_strip_quote_deep( $arr ) {
	if ( is_string( $arr ) ) {
		return wpcf8_strip_quote( $arr );
	}
	...
	if ( is_array( $arr ) ) {
		...
		foreach ( $arr as $key => $text ) {
			$result[$key] = wpcf8_strip_quote_deep( $text );
		}
...
function wpcf8_normalize_newline( $text, $to = "\n" ) {
	if ( ! is_string( $text ) ) {
		return $text;
	}
...
function wpcf8_normalize_newline_deep( $arr, $to = "\n" ) {
	if ( is_array( $arr ) ) {
		$result = array();

		foreach ( $arr as $key => $text ) {
			$result[$key] = wpcf8_normalize_newline_deep( $text, $to );
		}

		return $result;
	}

	return wpcf8_normalize_newline( $arr, $to );
}

function wpcf8_strip_newline( $str ) {
	$str = (string) $str;
	$str = str_replace( array( "\r", "\n" ), '', $str );
	return trim( $str );
}

function wpcf8_canonicalize( $text, $strto = 'lower' ) {
 ...
 */
function wpcf8_is_name( $string ) {
	return preg_match( '/^[A-Za-z][-A-Za-z0-9_:.]*$/', $string );
}

function wpcf8_sanitize_unit_tag( $tag ) {
	$tag = preg_replace( '/[^A-Za-z0-9_-]/', '', $tag );
	return $tag;
}

function wpcf8_is_email( $email ) {
	$result = is_email( $email );
	return apply_filters( 'wpcf8_is_email', $result, $email );
}

function wpcf8_is_url( $url ) {
	$result = ( false !== filter_var( $url, FILTER_VALIDATE_URL ) );
	return apply_filters( 'wpcf8_is_url', $result, $url );
}

function wpcf8_is_tel( $tel ) {
	$result = preg_match( '%^[+]?[0-9()/ -]*$%', $tel );
	return apply_filters( 'wpcf8_is_tel', $result, $tel );
}

function wpcf8_is_number( $number ) {
	$result = is_numeric( $number );
	return apply_filters( 'wpcf8_is_number', $result, $number );
}

function wpcf8_is_date( $date ) {
	...
	return apply_filters( 'wpcf8_is_date', $result, $date );
}

function wpcf8_is_mailbox_list( $mailbox_list ) {
...
function wpcf8_is_email_in_domain( $email, $domain ) {
	$email_list = wpcf8_is_mailbox_list( $email );
...
function wpcf8_is_email_in_site_domain( $email ) {
	...
	if ( wpcf8_is_email_in_domain( $email, $site_domain ) ) {
		return true;
	}
	...
	if ( preg_match( '%^https?://([^/]+)%', $home_url, $matches ) ) {
		...
		&& wpcf8_is_email_in_domain( $email, $site_domain ) ) {
			return true;
		}
	}
...
function wpcf8_antiscript_file_name( $filename ) {
...
function wpcf8_mask_password( $text, $length_unmasked = 0 ) {
	$length = strlen( $text );
