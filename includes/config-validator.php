...
class WPCF8_ConfigValidator {
	...
	public function validate() {
		...
		do_action( 'wpcf8_config_validator_validate', $this );
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		if ( $form_tags ) {
			$form_tag = new wpcf8_FormTag( $form_tags[0] );
			...
			if ( wpcf8_form_tag_supports( $form_tag->type, 'selectable-values' ) ) {
				if ( $form_tag->pipes instanceof wpcf8_Pipes ) {
					if ( $do_not_heat ) {
				...
				if ( $last_item && wpcf8_is_mailbox_list( $last_item ) ) {
			...
			$tagname = preg_replace( '/^wpcf8\./', '_', $tagname ); // for back-compat
	...
	public function detect_multiple_controls_in_label( $section, $content ) {
		...
		if ( preg_match_all( $pattern, $content, $matches ) ) {
			$form_tags_manager = wpcf8_FormTagsManager::get_instance();
			...
			foreach ( $matches[1] as $insidelabel ) {
				...
				foreach ( $tags as $tag ) {
					...
					$is_multiple_controls_container = wpcf8_form_tag_supports(
	...
	public function validate_mail( $template = 'mail' ) {
		...
		$body = new wpcf8_MailTaggedText( $body,
			array( 'callback' => $callback ) );
	...
	public function detect_invalid_mailbox_syntax( $section, $content, $args = '' ) {
		...
		if ( ! wpcf8_is_mailbox_list( $content ) ) {
			return $this->add_error( $section,
