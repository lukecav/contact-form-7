( function( $ ) {
	...
	$( function() {
		...
		updateWelcomePanel = function( visible ) {
			$.post( ajaxurl, {
				action: 'wpcf8-update-welcome-panel',
				visible: visible,
		...
		$( 'input:checkbox.toggle-form-table' ).click( function( event ) {
			$( this ).wpcf8ToggleFormTable();
		} ).wpcf8ToggleFormTable();
		...
		$( '[data-config-field]' ).change( function() {
			...
			$( this ).closest( 'form' ).find( '[data-config-field]' ).each( function() {
				data.push( {
					'name': $( this ).attr( 'name' ).replace( /^wpcf8-/, '' ).replace( /-/g, '_' ),
					'value': $( this ).val()
				});
			});
			...
			$.ajax( {
				method: 'POST',
				url: _wpcf8.apiSettings.root +
					'contact-form-8/v1/contact-forms/' + postId,
				beforeSend: function( xhr ) {
					xhr.setRequestHeader( 'X-WP-Nonce', _wpcf8.apiSettings.nonce );
				},
				data: data
			} ).done( function( response ) {
				_wpcf8.configValidator.errors = response.config_errors;
				$.wpcf8UpdateConfigErrors();
			} );
		} );
		...
		$( window ).on( 'beforeunload', function( event ) {
			...
			$( '#wpcf8-admin-form-element :input[type!="hidden"]' ).each( function() {
			...
			if ( changed ) {
				event.returnValue = _wpcf8.saveAlert;
				return _wpcf8.saveAlert;
			}
		} );

		$( '#wpcf8-admin-form-element' ).submit( function() {
	...
	$.fn.wpcf8ToggleFormTable = function() {
	...
	$.wpcf8UpdateConfigErrors = function() {
		var errors = _wpcf8.configValidator.errors;
		var errorCount = {
			total: 0,
		};
			...
			if ( errors[ section ] ) {
				...
				$.each( errors[ section ], function( i, val ) {
					...
					if ( val.link ) {
						...
						} ).text( _wpcf8.configValidator.howToCorrect ) );
		...
		$( '#contact-form-editor-tabs > li' ).each( function() {
			...
			if ( errorCount[tab] ) {
				...
				if ( 1 < errorCount[tab] ) {
					var manyErrorsInTab = _wpcf8.configValidator.manyErrorsInTab
						.replace( '%d', errorCount[tab] );
					$tabPanelError.append( manyErrorsInTab );
				} else {
					$tabPanelError.append( _wpcf8.configValidator.oneErrorInTab );
				}
			}
		} );
		...
		if ( errorCount.total ) {
			...
			if ( 1 < errorCount.total ) {
				$warning.append(
					_wpcf8.configValidator.manyErrors.replace( '%d', errorCount.total )
				);
			} else {
				$warning.append( _wpcf8.configValidator.oneError );
			}

			var $link = $( '<a></a>' ).attr( {
				'href': _wpcf8.configValidator.docUrl,
				'class': 'external dashicons dashicons-external'
			} ).append( $( '<span></span>' ).attr( {
				'class': 'screen-reader-text'
			} ).text( _wpcf8.configValidator.howToCorrect ) );
