( function( $ ) {
	...
	$( 'form.tag-generator-panel .control-box :input' ).change( function() {
		...
		_wpcf8.taggen.normalize( $( this ) );
		_wpcf8.taggen.update( $form );
	} );

	$( 'input.insert-tag' ).click( function() {
		var $form = $( this ).closest( 'form.tag-generator-panel' );
		var tag = $form.find( 'input.tag' ).val();
		_wpcf8.taggen.insert( tag );
		tb_remove(); // close thickbox
		return false;
	} );

	_wpcf8.taggen.update = function( $form ) {
		...
		if ( $.isFunction( _wpcf8.taggen.update[ id ] ) ) {
			return _wpcf8.taggen.update[ id ].call( this, $form );
		}
		...
		$form.find( 'input.tag' ).each( function() {
			...
			var components = _wpcf8.taggen.compose( tag_type, $form );
			$( this ).val( components );
		} );
	...
	_wpcf8.taggen.update.captcha = function( $form ) {
		var captchac = _wpcf8.taggen.compose( 'captchac', $form );
		var captchar = _wpcf8.taggen.compose( 'captchar', $form );

		$form.find( 'input.tag' ).val( captchac + ' ' + captchar );
	};

	_wpcf8.taggen.compose = function( tagType, $form ) {
	...
	_wpcf8.taggen.normalize = function( $input ) {
		var val = $input.val();
		...
		if ( $input.is( ':checkbox.exclusive' ) ) {
			_wpcf8.taggen.exclusiveCheckbox( $input );
		}
	};

	_wpcf8.taggen.exclusiveCheckbox = function( $cb ) {
		if ( $cb.is( ':checked' ) ) {
			$cb.siblings( ':checkbox.exclusive' ).prop( 'checked', false );
		}
	};

	_wpcf8.taggen.insert = function( content ) {
		$( 'textarea#wpcf8-form' ).each( function() {
			this.focus();
