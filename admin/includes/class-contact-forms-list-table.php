...
class WPCF7_Contact_Form_List_Table extends WP_List_Table {
	...
	function prepare_items() {
		...
		$this->items = WPCF8_ContactForm::find( $args );

		$total_items = WPCF8_ContactForm::count();
	...
	function get_bulk_actions() {
		$actions = array(
			'delete' => __( 'Delete', 'contact-form-8' ),
		);
	...
	function column_title( $item ) {
		$url = admin_url( 'admin.php?page=wpcf8&post=' . absint( $item->id() ) );
		$edit_link = add_query_arg( array( 'action' => 'edit' ), $url );

		$output = sprintf(
			'<a class="row-title" href="%1$s" title="%2$s">%3$s</a>',
			esc_url( $edit_link ),
			esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;', 'contact-form-8' ),
				$item->title() ) ),
			esc_html( $item->title() )
		);
		...
		&& current_user_can( 'wpcf8_edit_contact_form', $item->id() ) ) {
		...
		$actions = array(
			'edit' => sprintf( '<a href="%1$s">%2$s</a>',
				esc_url( $edit_link ),
				esc_html( __( 'Edit', 'contact-form-8' ) ) ) );

		if ( current_user_can( 'wpcf8_edit_contact_form', $item->id() ) ) {
			$copy_link = wp_nonce_url(
				add_query_arg( array( 'action' => 'copy' ), $url ),
				'wpcf8-copy-contact-form_' . absint( $item->id() ) );

			$actions = array_merge( $actions, array(
				'copy' => sprintf( '<a href="%1$s">%2$s</a>',
					esc_url( $copy_link ),
					esc_html( __( 'Duplicate', 'contact-form-8' ) )
				),
			) );
		}
	...
	function column_date( $item ) {
		...
		if ( $time_diff > 0 && $time_diff < 24*60*60 ) {
			$h_time = sprintf(
				__( '%s ago', 'contact-form-8' ), human_time_diff( $time ) );
		} else {
			$h_time = mysql2date( __( 'Y/m/d', 'contact-form-8' ), $m_time );
		}
